# A-DAP v0.4.0 — Precedence Record

This file records the public precedence anchor for A-DAP v0.4.0.
...
```text
A-DAP v0.4.0 — Externalization Baseline
```
...
A-DAP v0.4.0 is the public externalization baseline after the repository’s repositioning around affected-party contestability, scope boundaries, verification topology, and v0-to-v1 hardening roadmap.
...
The release may include conceptual notes, proof-of-concept scripts, adversarial stress tests, roadmap documentation, and precedence artifacts.
...
It records precedence for the specific A-DAP v0.4.0 formulation, including:
...
ROADMAP v0 → v1
precedence record for the public release
```
...
```text
A-DAP-v0.4.0-externalization-baseline.zip
```

Recommended hash file:

```text
A-DAP-v0.4.0-SHA256.txt
```
...
```text
A-DAP-v0.4.0-SHA256.txt.asc
```

or:

```text
A-DAP-v0.4.0-SHA256.txt.minisig
```
...
A-DAP-v0.4.0.tsq
A-DAP-v0.4.0.tsr
A-DAP-v0.4.0-timestamp-receipt.json
```
...
```text
1. The A-DAP v0.4.0 repository state existed in the released form.
...
A-DAP v0.4.0 should be interpreted under the following boundary:
...
```text
1. Download the release archive for tag v0.4.0.
...
This record anchors the public existence of A-DAP v0.4.0 as an externalization baseline.
